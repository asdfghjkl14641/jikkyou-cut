...
- **コメント分析画面(MVP)** — URL DL → 編集画面の間に「盛り上がりスコアグラフ + 区間選択」画面を挟む。詳細は `docs/COMMENT_ANALYSIS_DESIGN.md` 参照。チャットリプレイ取得(yt-dlp)+ コメント密度 + キーワード出現で 2 要素スコア計算 + 自動候補抽出 + 手動ドラッグ区間選択 + 編集画面に区間情報を渡す
