...
- **2026-04-30** シーク時のキュー一覧自動スクロール(現状不安定 → 解決) — `findCueIndexForScroll` で **キュー間ギャップ位置** にシークしても直前のキューにスクロール、冒頭・末尾の無音位置でも先頭/末尾キューへフォールバック。再生中追従は撤廃して seek 起点の片方向プッシュ(`seekNonce`)に再設計
