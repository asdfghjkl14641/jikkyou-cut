...
- **Phase B-3: 話者プリセット** — 話者ごとに字幕スタイルを割り当て可能に。SubtitleSettings の活性スタイル選択を「話者 → スタイルID」マップに拡張し、ASS 生成では cue.speaker を見て該当スタイルを引き当てる(なければ既定スタイル)。※話者ラベル管理（リネーム機能）もここで扱う。
...
- 🟡 **検討中: 範囲選択での一括話者変更** — 複数キューを一気に同じ話者に変更
