...
- **Phase B-2: 話者ラベル管理** — `speaker_0` / `speaker_1` を「みのる」「ヤマダ」のような表示名に**リネームできる UI** を追加。store + project file(`<basename>.jcut.json`)に label マップを永続化し、キュー一覧の話者バッジ・SRT プレフィックス・(将来の)字幕焼き込みでラベルを使う
- **Phase B-3: 話者プリセット** — 話者ごとに字幕スタイルを割り当て可能に。SubtitleSettings の活性スタイル選択を「話者 → スタイルID」マップに拡張し、ASS 生成では cue.speaker を見て該当スタイルを引き当てる(なければ既定スタイル)
...
- **話者分離 UI 拡張(色分け / フィルタ)** — 話者ごとの色分け / 話者フィルタ(「話者1のキューだけ表示」)。Phase B-2/B-3 のラベル管理 + プリセットが入った後の磨き候補
